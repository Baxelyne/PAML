...
def metrics(X, y_true1, y_true2, y_true3,
            y_pred1, y_pred2, y_pred3, measuring_metrics, dataManager):
    precision = -1.0
    recall = -1.0
    f1 = -1.0
    ...
    for i in range(len(y_true1)):
        ...
        y = [str(dataManager.id2label[val]) for val in y_true1[i] if val != dataManager.label2id[dataManager.PADDING]]
        y_hat = [str(dataManager.id2label[val]) for val in y_pred1[i] if
    ...
    for i in range(len(y_true2)):
        x = [str(dataManager.id2token[val]) for val in X[i] if val != dataManager.token2id[dataManager.PADDING]]
        y = [str(dataManager.id2label[val]) for val in y_true2[i] if val != dataManager.label2id[dataManager.PADDING]]
        y_hat = [str(dataManager.id2label[val]) for val in y_pred2[i] if
                 val != dataManager.label2id[dataManager.PADDING]]  # if val != 5
    ...
    for i in range(len(y_true3)):
        x = [str(dataManager.id2token[val]) for val in X[i] if val != dataManager.token2id[dataManager.PADDING]]
        y = [str(dataManager.id2label[val]) for val in y_true3[i] if val != dataManager.label2id[dataManager.PADDING]]
        y_hat = [str(dataManager.id2label[val]) for val in y_pred3[i] if
                 val != dataManager.label2id[dataManager.PADDING]]  # if val != 5
