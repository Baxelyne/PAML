...
class DataManager:
    ...
    def buildVocab(self, train_path):
        df_train = read_csv_(train_path, names=["token", "label1", "label2", "label3"],delimiter=self.configs.delimiter)
    ...
    def nextBatch(self, X, y1, y2, y3, start_index):
        last_index = start_index + self.batch_size
        X_batch = list(X[start_index:min(last_index, len(X))])
        y_batch1 = list(y1[start_index:min(last_index, len(X))])
        y_batch2 = list(y2[start_index:min(last_index, len(X))])
        y_batch3 = list(y3[start_index:min(last_index, len(X))])
        if last_index > len(X):
            left_size = last_index - (len(X))
            for i in range(left_size):
                index = np.random.randint(len(X))
                X_batch.append(X[index])
                y_batch1.append(y1[index])
                y_batch2.append(y2[index])
                y_batch3.append(y3[index])
    ...
    def getTrainingSet(self, train_val_ratio=0.9):
        df_train = read_csv_(self.train_file, names=["token", "label1", "label2", "label3"],delimiter=self.configs.delimiter)

        # map the token and label into id
        df_train["token_id"] = df_train.token.map(lambda x: -1 if str(x) == str(np.nan) else self.token2id[x])
        df_train["label1_id"] = df_train.label1.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
        df_train["label2_id"] = df_train.label2.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
        df_train["label3_id"] = df_train.label3.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
    ...
    def getValidingSet(self):
        df_val = read_csv_(self.dev_file, names=["token", "label1", "label2", "label3"],delimiter=self.configs.delimiter)

        df_val["token_id"] = df_val.token.map(lambda x: self.mapFunc(x, self.token2id))
        df_val["label1_id"] = df_val.label1.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
        df_val["label2_id"] = df_val.label2.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
        df_val["label3_id"] = df_val.label3.map(lambda x: -1 if str(x) == str(np.nan) else self.label2id[x])
    ...
    def getTestingSet(self):
        ...
        X_test_id, y_test_psyduo_label1 = self.prepare(df_test["token_id"], df_test["token_id"],
                                                      return_psyduo_label=True)
        _, y_test_psyduo_label2 = self.prepare(df_test["token_id"], df_test["token_id"],
                                                      return_psyduo_label=True)
        _, y_test_psyduo_label3 = self.prepare(df_test["token_id"], df_test["token_id"],
                                                      return_psyduo_label=True)
        X_test_token, _ = self.prepare(df_test["token"], df_test["token"])

        self.logger.info("\ntesting set size: %d\n" % (len(X_test_id)))
        return X_test_id, y_test_psyduo_label1,  y_test_psyduo_label2,  y_test_psyduo_label3, X_test_token
