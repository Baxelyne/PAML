...
class Configer:
    def __init__(self, config_file='system.config'):
        ...
        the_item = 'position_size'
        if the_item in config:
            self.position_size = int(config[the_item])
        the_item = 'position_dim'
        if the_item in config:
            self.position_dim = int(config[the_item])
    ...
    def show_data_summary(self, logger):
        ...
        logger.info("     position       size: %s" % (self.position_size))
        logger.info("     position        dim: %s" % (self.position_dim))
